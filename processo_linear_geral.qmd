...
Então, pela equação (@eq-covariancia), identificando $\psi_j=\phi^j$,teremos que
...
e, considerando que $|\dot{B}|>1$, o processo será estacionário se $(\phi_1,\phi_2)$ pertence ao triângulo delimitado pelos vértices (-2,-1), (0,1), (2,-1). É interessante notarmos que, se $\phi_2<0$, as raízes de $\phi(B)$ serão um par de números complexos conjugados. 
...
t-j})
\end{align}$$
e, como $\gamma(h)=E(x_t x_{t-h})$, teremos
$$\begin{align}
\gamma(h)&=\sum_{j=1}^p \phi_j \gamma(h-j)
\end{align}$$
Dividindo ambos os lados por $\gamma(0)$, teremos
$$\begin{align}
...
Considere o processo $AR(2)$ abaixo:
	$$x_{t}=\phi_1y_{t-1}+\phi_2y_{t-2}+\varepsilon_t.$$
...
$$\begin{align*}
	...
	\end{align*}$$
Dividindo ambos os lados por $\gamma(0)$ teremos		$$\rho(1)=\phi_1+\phi_2\rho(1),$$
logo,
...
Com os valores de $\rho(1)$ e $\rho(2)$, podemos encontrar $\rho(3)$:
...
$$\rho(h)=\sum_{j=1}^p c_j\left(\frac{1}{\dot{B}_j}\right)^h$$
onde $\dot{B}$, novamente, é a raiz de $\phi(B)=1-\sum_{j=1}^p \phi_j B^j$. 
